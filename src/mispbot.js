  ...
  if (command === "misp") {
    ...
    let getioc = async () => {
      let response = await axios
        .get(
          `https://${config.mispserver}/events/index/searchall:${combinedArgs}`,
          {
            headers: {
              Authorization: `${config.misptoken}`,
              responseType: "json"
            }
          }
        )
    ...
    for (element in getfindings) {
      console.log(element);
      msg.channel.send(`https://${config.mispserver}/events/view/${
...
if (command === "mispevent") {
  ...
  for (element in getfindings) {
    console.log(element);
    msg.channel.send(`https://${config.mispserver}/events/view/${getfindings[element].id}
    Date = ${getfindings[element].date}
    Title = ${getfindings[element].info}`);
  }
}
